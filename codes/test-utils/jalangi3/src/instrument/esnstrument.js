...
//        );
    ...
    // ES6 compatibility: helper function to determine whether it is strict function
    function isStrictFunction(node) {
	// arrow func always strict
        if (node.type === 'ArrowFunctionExpression') return true;

	if (node.params && node.params.length > 0) {
	   return node.params.some(function(p) {
	       // both argument types guarantee strict function
               return p.type === 'RestElement' || p.type == 'AssignmentPattern';
	   });
	}
	return false;
    }
	...
	// added ES6: arrow functions are always strict, no arguments.callee
	if (isStrictFunction(node)) {
	    ret = replaceInStatement(
		    logFunctionEnterFunName + "(" + RP + "1, null, this, null)",
		    getIid()
	    );
	} else {
       	    ret = replaceInStatement(
    ...
    function syncDefuns(node, scope, isScript) {
        var ret = [], ident;
	// added ES6: strict function has no arguments binding
        if (!isScript && !isStrictFunction(node)) {
            // end mod
            if (!Config.INSTR_TRY_CATCH_ARGUMENTS || Config.INSTR_TRY_CATCH_ARGUMENTS(node)) {
                if (!Config.INSTR_INIT || Config.INSTR_INIT(node)) {
...
//    }
	...
	// added for ES6 compatibility
	'ArrowFunctionExpression': mergeBodies,
	// end mod
    };
    ...
    var visitorRRPre = {
	 ...
	 // added for ES6 compatibility
	 'ArrowFunctionExpression': setScope,
	 // end mod
    ...
    var visitorRRPost = {
		...
		// added additional ES6 safeguard
                if (def.init !== null && def.id.type === 'Identifier') {
	            // end mod
		    // added accomodation for 'const' and 'let' types
		    var lhs = node.kind === 'var' ? def.id : 
				createIdentifierAst('undefined');
		    // end mod
                    var init = wrapWrite(def.init, createLiteralAst(def.id.name), def.init, lhs, false, scope.isGlobal(def.id.name), true);
...
//            return ret1;
	...
	'ArrowFunctionExpression': function (node, context) {
		// if body is a block, treat like Function expression
		if (node.body.type === 'BlockStatement') {
			node.body.body = instrumentFunctionEntryExit(node, node.body.body);
		}
		// if body is an expression (concise arrow, it is implicitly a return

		// arrow don't have 'this' or 'argument'
		// so no N_LOG_FUNCTION_LIT_T wrap
		scope = scope.parent;
		return node;
	},
	// end mod
...
//                var ret = prependScriptBody(node, body);
    ...
    function addScopes(ast) {
	    ...
	    // added for ES6 compatibility
	        ...
	        else if (node.type === 'AssignmentPattern' && param.left.type === 'Identifier') {
			if (param.left.name === fromName) { 
				param.name = toName;
			}
			currentScope.addVar(param.left.name, 'arg');
		}
		else if (node.type === 'RestElement' && param.argument.type === 'Identifier') {
			if (param.argument.name === fromName) {
				param.argument.name = toName;
			}
			currentScope.addVar(param.argument.name, 'arg');
		}
	    // RestElement, AssignmentPattern, ObjectPattern, ArrayPattern
    	    // are left unregistered and will fall through as globals,
            // which is safe for jalangi's purposes
            // end mod
        }
	 ...
	 function handleVarDecl(node) {
    	   // add compatibility for 'const' and 'let' types
	    var kind = node.kind === 'var' ? 'var' : 'lexical';
	    MAP(node.declarations, function(decl) {
		    if (decl.id.type === 'Identifier') {
			    currentScope.addVar(decl.id.name, kind);
		    }
		    // destructuring patterns left unregistered intentionally
	    });
            // end mod
	 }
	    ...
	    // end mod
	    // need to accomodate for 'const' and 'et' types
            // 'VariableDeclarator': handleVar,
	    'VariableDeclaration': handleVarDecl,
	     // end mod
        ...
        var visitorPost = {
	     ...
	     // added ES6 capacity
	    'ArrowFunctionExpression': popScope,
	    // end mod
	...
	// added compatibility for most recent versions (e.g. ES2017+)
        var newAst = acorn.parse(code, {locations: true, ecmaVersion: 'latest' });
	// end mod
...
//        console.log(JSON.stringify(newAst,null,"  "));
    ...
    function instrumentCode(options) {
		...
		// modified to not use esotope
                // var newCode = esotope.generate(newAst, {comment: true ,parse: acorn.parse});
		var newCode = escodegen.generate(newAst, { comment: true, parse: acorn.parse});
                code = newCode + "\n" + noInstr + "\n";
            } catch(ex) {
